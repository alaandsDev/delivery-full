...
export default function ProdutosPage() {
  ...
  useEffect(() => { if (store) fetchData(); }, [store, fetchData]);
  ...
  function openEditProduct(prod: Product) {
    ...
    setForm({ name: prod.name, description: prod.description ?? '', priceInput: (prod.priceCents / 100).toFixed(2).replace('.', ','), categoryId: prod.categoryId });
    setFormError(null);
    setModalOpen(true);
  }
  ...
  async function toggleActive(prod: Product) {
    await fetch(`${API}/products/${prod.id}`, { method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ isActive: !prod.isActive }) });
    await fetchData();
  }
  ...
  async function saveCategory() {
    ...
    setSavingCat(true);
    await fetch(`${API}/categories`, { method: 'POST', headers: authHeaders(), body: JSON.stringify({ storeId: store.id, name: catName.trim() }) });
    await fetchData();
    setCatName(''); setCatModalOpen(false); setSavingCat(false);
  }
  ...
  return (
    <div className="painel-layout">
      ...
      <main className="painel-content">
        ...
        {loading ? <div className="painel-loading"><div className="cardapio-spinner" /></div> : (
        ...
        <div className="cardapio-overlay" onClick={() => setModalOpen(false)}>
          <div className="painel-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '16px 0' }}>
              <label className="cardapio-label">Nome *<input className="cardapio-input" value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="Ex: X-Burguer" /></label>
              <label className="cardapio-label">Descrição<input className="cardapio-input" value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="Ingredientes, tamanho..." /></label>
              <label className="cardapio-label">Preço (R$) *<input className="cardapio-input" value={form.priceInput} onChange={(e) => handlePriceInput(e.target.value)} placeholder="Ex: 28,90" inputMode="numeric" /></label>
              <label className="cardapio-label">Categoria *
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="cardapio-btn-primary" onClick={saveProduct} disabled={saving}>{saving ? 'Salvando...' : 'Salvar'}</button>
        ...
        <div className="cardapio-overlay" onClick={() => setCatModalOpen(false)}>
          <div className="painel-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: '16px 0' }}>
              <label className="cardapio-label">Nome<input className="cardapio-input" value={catName} onChange={(e) => setCatName(e.target.value)} placeholder="Ex: Hambúrgueres, Bebidas..." /></label>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button className="cardapio-btn-primary" onClick={saveCategory} disabled={savingCat || !catName.trim()}>{savingCat ? 'Salvando...' : 'Criar categoria'}</button>
